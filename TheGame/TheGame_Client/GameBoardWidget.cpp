    ...
    , m_username(username)
{
 ...
 ui->stack1->installEventFilter(this);
    ui->stack2->installEventFilter(this);
  ui->stack3->installEventFilter(this);
    ui->stack4->installEventFilter(this);
...
bool GameBoardWidget::eventFilter(QObject* obj, QEvent* event)
{
    if (event->type() == QEvent::MouseButtonPress) {
        if (obj == ui->stack1) {
            onStack1Clicked();
    return true;
        } else if (obj == ui->stack2) {
            onStack2Clicked();
            return true;
        } else if (obj == ui->stack3) {
   onStack3Clicked();
   return true;
 } else if (obj == ui->stack4) {
          onStack4Clicked();
      return true;
  }
    }
...
void GameBoardWidget::updateTurnInfo(const QJsonObject& state)
{
 int currentIdx = state["currentPlayer"].toInt();
    ...
    ui->lblStatus->setStyleSheet("font-size: 16pt; font-weight: bold; color: #ffd700; background-color: transparent;");
            ...
            ui->stack1->setEnabled(true);
      ui->stack2->setEnabled(true);
  ui->stack3->setEnabled(true);
            ui->stack4->setEnabled(true);
  ...
  ui->lblStatus->setText("⏳ Asteptam jucatorul: " + currentPlayer);
      ...
      ui->stack1->setEnabled(false);
ui->stack2->setEnabled(false);
          ui->stack3->setEnabled(false);
   ui->stack4->setEnabled(false);
